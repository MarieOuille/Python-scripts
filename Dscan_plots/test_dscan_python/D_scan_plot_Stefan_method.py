...
# attention : spect_phase_nu MOINS correction !!! signe -
...
maxI = max(pulse_I)
maxind = 2039
#[maxI,maxind] = max(pulse_I)
pulse_tau = np.diff(t[abs(np.diff(np.sign(pulse_I - 0.5*(max(pulse_I)))))==2])
#pulse_tau = np.interp(t[maxind:np.size(t)-1], pulse_I[maxind:np.size(pulse_I)-1],0.5*maxI) - np.interp(t[0:maxind], pulse_I[0:maxind],0.5*maxI) #more precise meas. using interpolation
...
#maxI = max(pulse_I_corr)
#for i, value in enumerate (pulse_I_corr):
#    if pulse_I_corr[i] == maxI:
#        maxind = i
#        break
#[maxI,maxind] = max(pulse_I_corr);
pulse_tau_corr = np.diff(t[abs(np.diff(np.sign(pulse_I_corr - 0.5*(max(pulse_I_corr)))))==2])
#pulse_tau_corr = interp1(pulse_I_corr(maxind:end),t(maxind:end),0.5*maxI) ...
#                - interp1(pulse_I_corr(1:maxind),t(1:maxind),0.5*maxI); %more precise meas. using interpolation
...
#[maxI,maxind] = max(pulse_I_FTL);
pulse_tau_FTL = np.diff(t[abs(np.diff(np.sign(pulse_I_FTL - 0.5*(max(pulse_I_FTL)))))==2])
#pulse_tau_FTL = interp1(pulse_I_FTL(maxind:end),t(maxind:end),0.5*maxI) ...
#               - interp1(pulse_I_FTL(1:maxind),t(1:maxind),0.5*maxI); %more precise meas. using interpolation
print('FTL duration = '+str(pulse_tau_FTL)+' fs FWHM.')
print('with peak intensity gain of a factor = '+str(max(pulse_I_FTL)/max(pulse_I))+'.')


## Compare to the d-scan reconstruction - works !
#% figure;
#% plot(t,real(pulse_t)/max(abs(pulse_t)))
#% hold on
#% plot(pulse_T, pulse_Car)
#% xlim([-30,30])
#% 
#% figure;
#% plot(t,pulse_I/max(abs(pulse_I)))
#% hold on
#% plot(pulse_T, pulse_Env.^2)
#% xlim([-30,30])
#
...
plt.figure()
plt.plot(t[2:np.size(t)],pulse_t.real, 'navy', label='initial E field')
plt.plot(t[2:np.size(t)],pulse_t_corr.real, 'r', label= 'corrected E field')
plt.xlim([-30,30])
