"""
 ...
 phase values are recorded in an array (old versions) or do  phases/4096*2*np.pi  (newer version)
...
def fringeezz_load_h5_data_log(file, filepath, print_structure=True):
    ...
    return config[0].decode('UTF-8'), timestamps, phases/4096*2*np.pi   #phases*1e-4  for older versions of the Fringeezz
